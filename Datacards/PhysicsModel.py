...
class CPHiggsModel(PhysicsModel):
    "Define parameters of interest"
    def doParametersOfInterest(self):
        """Create POI and other parameters, and define the POI set."""
        # --- Only POI is strength of CP-even signal ---
        self.modelBuilder.doVar("re[1,0,1]");
        self.modelBuilder.doSet("POI","re")
    def getYieldScale(self,bin,process):
        if process=="sig0":
            return "re"
        elif process=="sigpi2":
            self.modelBuilder.factory_('expr::%s("1 - @0", %s)' % ("ro", "re"))
            return "ro"
        else: return 1
